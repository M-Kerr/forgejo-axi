...
Each lane runs the whole issue family; `label` create, list, edit, and delete; `repo view` and `repo create`; `api --paginate` across a genuine page boundary; `pr` create, list, find, view, update, checks, mergeability, merge, merged, reviews, and diff; and runner-free `run` probes. The review probe submits the one verdict Forgejo lets an author record on their own pull request — a `COMMENT` review carrying an inline comment — and then asserts the reviewer, the verdict, and the file anchor come back through `pr reviews`. On a host advertising the runs capability, `run list` must decode Forgejo's real `{workflow_runs}` envelope and have its `--status` and `--branch` filters accepted, and a missing run must map to `NOT_FOUND`; on a host without the route, the family must report itself unsupported from the probe.

The `repo create` probe drives both creation routes with a disposable repository named after the run: one under the lane repository's owner and one under the authenticated login, which collapse into a single user-route probe when the lane repository belongs to that login. For each route it asserts the host serves the repository back under that name, that a rerun is `created: false` with an empty `differs`, that requesting the other visibility is reported in `differs` and not applied, and that `repo view` returns the `clone_url` rebuilt from the base URL and the host's `ssh_url` in either its `ssh://` or scp form. A create the token cannot perform records a failure for that route and the lane continues; it also asserts that omitting the visibility is refused before any request. Every repository the run created is deleted by address at the end, and one the token cannot delete is reported as leaked.
